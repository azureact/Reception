...
driver.find_element(
    By.XPATH,
    "//*[@id='html-body']/div[2]/div[2]/div/div[1]/div[1]/form/div[1]/div/input",
).send_keys(
    ""
)  # 登录页面
driver.find_element(
    By.XPATH,
    "//*[@id='html-body']/div[2]/div[2]/div/div[1]/div[1]/form/div[2]/div/input",
).send_keys(
    ""
)  # 被自动填写的Wikidot账号
driver.find_element(
    ...
    while True:
        if messages == []:
            ...
            if mes_temp[2] == "身份组更新完成":
                ...
                await mes_temp[0].author.remove_roles(
                    guild.get_role(1193187250553503866)
                )
...
def verify(ctx, wikidot_id: str):
    ...
    try:
        ...
        try:
            messages.append(
                [ctx, discord_id, f"此账户已绑定*{users_dic[discord_id]}*。"]
            )
            ...
            for i in users_dic:
                if i[0] == wikidot_id:
                    messages.append(
                        [ctx, discord_id, f"wikidot账号*{wikidot_id}*已被绑定。"]
                    )
                    return None
        ...
        driver.get(
            driver.find_element(
                By.XPATH, '//*[@id="page-content"]/div[2]/a[1]'
            ).get_attribute("href")
        )
        driver.implicitly_wait(3)
        driver.find_element(By.XPATH, '//*[@id="pm-subject"]').send_keys(
            "xxxDiscord服务器验证码"
        )  # wikidot网站名称
        code = "".join(random.sample(string.digits, 6))
        driver.find_element(By.XPATH, '//*[@id="editor-textarea"]').send_keys(
            f"你的验证码是{code}，五分钟之内有效。"
        )
        time.sleep(1)
        ...
        driver.close()
        messages.append(
            [ctx, discord_id, "验证码已发送，请在五分钟内输入验证码以完成验证。"]
        )
...
async def code_command(ctx, code: str):
    ...
    try:
        if code_dic[discord_id][1] == code:
            role_id = (
                1193187250553503866 if code_dic[discord_id][3] else 934286697942908968
            )
            guild = ctx.guild
